...
1. [**SnarkVM（AleoVM）**](./snarkvm.md)：链下执行环境，作为 Aleo 的虚拟机。SnarkVM 本质上是 AleoVM，它允许执行 Aleo 程序和生成零知识证明。通过利用 SnarkVM 和加密，任何人都可以以隐私保护的方式执行逻辑。

2. [**SnarkOS**](./snarkos.md)：Aleo 的区块链节点客户端，由 AleoBFT 共识机制提供支持。SnarkOS 实现 AleoBFT 协议，确保网络参与者之间的安全高效共识。
...
有关这些组件的更详细信息，请参考参考部分的 [规范说明](../specifications.md)，它涵盖了 SnarkVM 和 SnarkOS 的深度内容。

## Aleo 发展路线图

Aleo 不断改进其协议以增强隐私性、可扩展性和可用性。路线图概述了旨在加强网络基础设施、引入新功能和优化性能的关键举措和项目。通过遵循路线图，开发者和用户可以了解最新的发布信息，并参与 Aleo 生态系统的演变。

有关 Aleo 开发进度和未来里程碑的最新信息，请访问官方 [Aleo 发展路线图](https://aleo.org/roadmap/)。

## 是什么让 Aleo 具备无需许可性、隐私性和可编程性？

Aleo 网络架构的概述将有助于解释在学习部分介绍的概念。
...
有多个验证者（验证者 1、验证者 2、...、验证者 n），他们协调以确保网络的完整性和安全性。
